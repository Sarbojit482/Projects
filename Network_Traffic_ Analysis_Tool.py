...
# New Tool 1: DNS Query Detection
def detect_dns_queries(packets):
    dns_queries = []
    for packet in packets:
        if packet.haslayer(DNS) and packet[DNS].qr == 0:  # DNS query
            src_ip = packet[IP].src
            queried_domain = packet[DNSQR].qname.decode() if packet.haslayer(DNSQR) else "Unknown"
            dns_queries.append({"src_ip": src_ip, "queried_domain": queried_domain})
...
# New Tool 2: ARP Spoofing Detection
    ...
    for packet in packets:
        if packet.haslayer(ARP) and packet[ARP].op == 2:  # ARP is-at (response)
            ...
            if src_ip in seen_ips:
                if seen_ips[src_ip] != src_mac:
                    arp_spoofing.append({"src_ip": src_ip, "detected_mac": src_mac, "original_mac": seen_ips[src_ip]})
            else:
                seen_ips[src_ip] = src_mac

    if arp_spoofing:
        df_arp_spoofing = pd.DataFrame(arp_spoofing)
        logger.warning("\nPotential ARP Spoofing Detected:\n")
        logger.warning(tabulate(df_arp_spoofing, headers="keys", tablefmt="grid"))
